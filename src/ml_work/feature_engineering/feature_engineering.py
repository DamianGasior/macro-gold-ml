...
class FeatureEngineering:
    ...
    def currency_features(self, dataframe, *args):
        ...
        )  # sorting from the oldest on the top to the newest on the bottom, most of the fuction in pandas expect that the previous row is the earlierst one
        for ccy_pair in args:
            self._dataframe[f"{ccy_pair}_lag_1"] = dataframe[ccy_pair].shift(
                1
            )  # creating one column and moving it down by 1
            self._dataframe[f"{ccy_pair}_roll_5"] = (
                self._dataframe[f"{ccy_pair}_lag_1"].rolling(5).mean()
            )
            self._dataframe[f"{ccy_pair}_momentum_5"] = (
                self._dataframe[f"{ccy_pair}_lag_1"].rolling(5).mean().diff()
            )  # taking the values from lag_1 , caclulcation the rolling mean and then the difference between those
            self._dataframe[f"{ccy_pair}_std_5"] = (
                self._dataframe[f"{ccy_pair}_lag_1"].rolling(5).std()
            )
            self._dataframe[f"{ccy_pair}_roll_10"] = (
                self._dataframe[f"{ccy_pair}_lag_1"].rolling(10).mean()
            )
            self._dataframe[f"{ccy_pair}_momentum_10"] = (
                self._dataframe[f"{ccy_pair}_lag_1"].rolling(10).mean().diff()
            )
            self._dataframe[f"{ccy_pair}_std_10"] = (
                self._dataframe[f"{ccy_pair}_lag_1"].rolling(10).std()
            )
            self._dataframe[f"{ccy_pair}_change"] = self._dataframe[
                f"{ccy_pair}_lag_1"
            ].pct_change()
    ...
    def yield_spread_features(self, dataframe, yield1, yield2):
        ...
        self._dataframe[f"{yield1}_{yield2}_spread_lag_1"] = self._dataframe[
            f"{yield1}_{yield2}_spread"
        ].shift(1)
        self._dataframe[f"{yield1}_{yield2}_spread_roll_5"] = (
            self._dataframe[f"{yield1}_{yield2}_spread_lag_1"].rolling(5).mean()
        )
        self._dataframe[f"{yield1}_{yield2}_spread_momentum_5"] = (
            self._dataframe[f"{yield1}_{yield2}_spread_lag_1"].rolling(5).mean().diff()
        )
        self._dataframe[f"{yield1}_{yield2}_spread_std_5"] = (
            self._dataframe[f"{yield1}_{yield2}_spread_lag_1"].rolling(5).std()
        )
        self._dataframe[f"{yield1}_{yield2}_spread_roll_10"] = (
            self._dataframe[f"{yield1}_{yield2}_spread_lag_1"].rolling(10).mean()
        )
        self._dataframe[f"{yield1}_{yield2}_spread_momentum_10"] = (
            self._dataframe[f"{yield1}_{yield2}_spread_lag_1"].rolling(10).mean().diff()
        )
        self._dataframe[f"{yield1}_{yield2}_spread_std_10"] = (
            self._dataframe[f"{yield1}_{yield2}_spread_lag_1"].rolling(10).std()
        )
    ...
    def correlation_betwee_instrumentss(self, *args):
        for arg1, arg2 in itertools.combinations(args, 2):
            self._dataframe[f"corr_{arg1}_{arg2}"] = (
                (self._dataframe[f"{arg1}_change"])
                .rolling(5)
                .corr(self._dataframe[f"{arg2}_change"])
            )
    ...
    def basic_metrics(self, dataframe):
        ...
        for symbol in columns:
            ...
            self._dataframe[f"{symbol}_mom_5"] = (
                dataframe[symbol].shift(1) - dataframe[symbol].shift(6)
            ) / dataframe[symbol].shift(6)
            ...
            self._dataframe[f"{symbol}_vol_ratio"] = (
                returns_lag.rolling(5).std() / returns_lag.rolling(20).std()
            )  # volatility ratio, if short term vol will rise against the longer one, it will tell that somehting start to happen
            self._dataframe[f"{symbol}_vol_regime"] = (
                returns_lag.rolling(5).std() > returns_lag.rolling(20).std()
            ).astype(int)
    ...
    def ratios(self, dataframe, gold, dxy, us_yield, usd_eur, usd_pln, pl_yields):
        ...
        self._dataframe[f"{usd_eur}_vs_{us_yield}"] = dataframe[usd_eur].shift(1) / dataframe[
            us_yield
        ].shift(1)
        self._dataframe[f"{usd_pln}_vs_{pl_yields}"] = dataframe[usd_pln].shift(1) / dataframe[
            pl_yields
        ].shift(1)
